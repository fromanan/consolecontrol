...
{
    private const int _WORKER_UPDATE_FREQUENCY = 200;
    ...
    private void outputWorker_DoWork(object? sender, DoWorkEventArgs e)
    {
        ...
        {
            ReadLine(_outputReader, _outputWorker);
            Thread.Sleep(_WORKER_UPDATE_FREQUENCY);
        }
    }
    ...
    private void errorWorker_DoWork(object? sender, DoWorkEventArgs e)
    {
        ...
        {
            ReadLine(_errorReader, _errorWorker);
            Thread.Sleep(_WORKER_UPDATE_FREQUENCY);
        }
    }

    private static void ReadLine(TextReader? reader, BackgroundWorker worker)
    {
        //  Any lines to read?
        int count;
        ...
        do
        {
            ...
            count = reader?.Read(buffer, 0, 1024) ?? 0;
    ...
    {
        //  Fire process exited.
        FireProcessExitEvent(Process?.ExitCode ?? -1);
    ...
    {
        //  Get the event and fire it.
        ProcessEventHandler? theEvent = OnProcessInput;
        theEvent?.Invoke(this, new ProcessEventArgs(content));
    }
    ...
    {
        //  Get the event and fire it.
        ProcessEventHandler? theEvent = OnProcessExit;
    ...
    {
        if (!IsProcessRunning || _inputWriter is null)
            return;
